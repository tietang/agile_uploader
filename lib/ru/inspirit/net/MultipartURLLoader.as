package ru.inspirit.net
{
	...
	public class  MultipartURLLoader extends EventDispatcher
	{
		...
		public function addHeader(name:String, value:String):void {
			// Push additional headers .. This could be for example X-Requested-With so things like CakePHP will know it's an AJAX request
			urlRequest.requestHeaders.push(new URLRequestHeader(name, value));			
		}
		
		private function doSend():void
		{
			//var urlRequest:URLRequest = new URLRequest(); // now up top so it's global and we can use addHeader() 
			urlRequest.url = _path;
			//urlRequest.contentType = 'multipart/form-data; boundary=' + getBoundary();
			// seemed to not set from URLRequestHeader() ... I don't know. Have to look.
		...
		private function onComplete( event: Event ): void
		{
			removeListener();
			dispatchEvent( event );
			dispatchEvent(new DataEvent(DataEvent.UPLOAD_COMPLETE_DATA, event.bubbles, event.cancelable, _loader.data)); // one dude said this...can't seem to figure it
		}
